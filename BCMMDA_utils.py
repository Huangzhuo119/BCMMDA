...
def load_train_val_test_set(input_path,decompose,decompose_protein,unseen_smiles=False,k=3,split_random=True):
    ...
    if unseen_smiles:
        trainSmilesy, trainProteiny, trainLabely, valSmilesy, valProteiny, valLabely, testSmilesy, testProteiny, testLabely=split_unseen_drug(datasetSmiles,datasetProtein,datasetLabel)

    else:
        trainSmilesy, trainProteiny, trainLabely, valSmilesy, valProteiny, valLabely, testSmilesy, testProteiny, testLabely=split_train_val_test_set(datasetSmiles,datasetProtein,datasetLabel,split_random)

    if decompose == "bcm":
        trainSmiles, trainProtein, trainLabel=get_one_bcm(trainSmilesy,trainProteiny,trainLabely,decompose2=decompose_protein,k=k)
        valSmiles,valProtein,valLabel=get_one_bcm(valSmilesy,valProteiny,valLabely,decompose2=decompose_protein,k=k)
        testSmiles,testProtein,testLabel=get_one_bcm(testSmilesy,testProteiny,testLabely,decompose2=decompose_protein,k=k)
        # caseSmiles,caseProtein,caseLabel=get_one_bcm(smiles,protein,[],decompose2=decompose_protein,k=k)


    else:
        trainSmiles, trainProtein, trainLabel = get_one_ch(trainSmilesy, trainProteiny, trainLabely,k=k)
        valSmiles, valProtein, valLabel = get_one_ch(valSmilesy, valProteiny, valLabely,k=k)
        testSmiles, testProtein, testLabel = get_one_ch(testSmilesy, testProteiny, testLabely,k=k)
